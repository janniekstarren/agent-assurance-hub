...
- **AI gateway control + token metrics (Foundry / custom / MCP agents)** → **Azure API Management GenAI gateway** fronts the model endpoints you own: per-consumer token metrics to Application Insights (`llm-emit-token-metric`), prompt/completion logging to Azure Monitor, enforced token quotas (`llm-token-limit`) and inline **Azure AI Content Safety** (`llm-content-safety`), plus backend load-balancing + circuit breakers. **Boundary (the honest bit):** the gateway only sees traffic routed *through* it — **Copilot Studio + SharePoint agents call models via Microsoft's managed orchestrator, which does not traverse your gateway**, so their internal model usage stays opaque (PPAC credit reports remain the only cost source there). The Hub marks gateway-fronted agents **Enforced** vs passive App-Insights **Observed** in the Coverage view. Foundry AI-gateway integration + unified model API are **(preview)**; capability set varies by APIM tier.
